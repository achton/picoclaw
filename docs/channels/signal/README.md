...
| account               | string   | Yes      | Signal phone number in E.164 format (e.g. `+1234567890`)         |
| signal_cli_url        | string   | No       | signal-cli HTTP API URL (default: `http://localhost:8080`)       |
...
| reasoning_channel_id  | string   | No       | Optional chat ID (phone number or group ID) to receive the LLM's reasoning/thoughts separately |
...
**Keeping PII out of `config.json`:** any field can be moved to a separate [`.security.yml`](../../security_configuration.md) file alongside `config.json` — useful for keeping `account` and `allow_from` phone numbers out of version-controlled configs. Omit the field from `config.json` and set it under `channels.signal.*` in `.security.yml`. See [Security Configuration](../../security_configuration.md) for the full mechanism (including optional `enc://` encryption-at-rest).
...
The following fields support environment variable overrides:

| Variable                                         | Maps to                |
| ------------------------------------------------ | ---------------------- |
| `PICOCLAW_CHANNELS_SIGNAL_ENABLED`               | `enabled`              |
| `PICOCLAW_CHANNELS_SIGNAL_ACCOUNT`               | `account`              |
| `PICOCLAW_CHANNELS_SIGNAL_CLI_URL`               | `signal_cli_url`       |
| `PICOCLAW_CHANNELS_SIGNAL_ALLOW_FROM`            | `allow_from`           |
| `PICOCLAW_CHANNELS_SIGNAL_REASONING_CHANNEL_ID`  | `reasoning_channel_id` |

`group_trigger` must be set via JSON config (no env var).
...
Add the `channels.signal` block from the [Configuration](#configuration) section to your `config.json`, setting:

- `account` — the number you registered (E.164, same as Step 1)
- `signal_cli_url` — where signal-cli is reachable:
...
2. Confirm `account` matches the number registered in signal-cli.
